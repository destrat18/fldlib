...
#ifndef FLOAT_AFFINE_OPTION
typedef NumericalDomains::FloatZonotope float_fld;
typedef NumericalDomains::DoubleZonotope double_fld;
typedef NumericalDomains::LongDoubleZonotope long_double_fld;
#else // FLOAT_AFFINE_OPTION
typedef NumericalDomains::TFldlibZonotopeOption<old_float> float_fld;
typedef NumericalDomains::TFldlibZonotopeOption<old_double> double_fld;
typedef NumericalDomains::TFldlibZonotopeOption<old_long_double> long_double_fld;
#endif

#ifndef FLOAT_KEEP_DOUBLE
#define float float_fld
#define double double_fld
#define long_double long_double_fld
#endif

#define FBETWEEN(x,y) float_fld(old_float(x), old_float(y))
#define FBETWEEN_WITH_ERROR(x,y,errmin,errmax) float_fld(old_float(x), old_float(y), old_float(errmin), old_float(errmax))
#define DBETWEEN(x,y) double_fld((old_double) x, (old_double) y)
#define DBETWEEN_WITH_ERROR(x,y,errmin,errmax) double_fld((old_double) x, (old_double) y, (old_double) errmin, (old_double) errmax)
...
#define IPRINT(x) double_fld(x).lightPersist(#x":\t")
...
#define ICPRINT(name, x) double_fld(x).lightPersist(name":\t")
...
#define FFROM_STRING(x) float_fld(#x, float_fld::ValueFromString())
#define DFROM_STRING(x) double_fld(#x, double_fld::ValueFromString())
...
#define FLOAT_MERGE_ALL(ident, x, load)                                                          \
   ...
   double_fld::MergeBranches(_sourceFile##ident, _sourceLine##ident) << x;        \
...
#if !defined(FLOAT_AFFINE_OPTION) || !defined(FLOAT_SUPPORT_INT_DOMAIN)
...
#define FLOAT_MERGE_ALL(ident, x, load)                                                          \
         FLOAT_PRINT_CURRENT_PATH(ident)                                                         \
         _isCompleteFlow##ident = double_fld::MergeBranches(_sourceFile##ident, _sourceLine##ident) << x;\
      }                                                                                          \
...
#else // FLOAT_AFFINE_OPTION && FLOAT_SUPPORT_INT_DOMAIN
...
#define FLOAT_SPLIT_ALL(ident, merge, save) {                                                    \
   ...
   NumericalDomains::DAffine::PathExplorer _pathExplorer##ident(                                 \
        NumericalDomains::DAffine::ExecutionPath::queryMode(_oldPathExplorer##ident));           \
   NumericalDomains::DAffine::ExecutionPath::setCurrentPathExplorer(&_pathExplorer##ident);      \
   auto _mergeMemory##ident = NumericalDomains::FldlibBase::MergeMemory() >> merge;              \
   auto _saveMemory##ident = NumericalDomains::FldlibBase::SaveMemory() << save;                 \
   auto _oldSourceInfo##ident = NumericalDomains::DAffine::BaseFloatAffine::querySplitInfo();    \
   bool _doesIterate##ident;                                                                     \
   int _oldSplitBranchIdentifier##ident = NumericalDomains::FldlibBase::splitBranchIdentifier;   \
   int _currentSplitBranchIdentifier##ident = ++NumericalDomains::FldlibBase::numberOfBranches;  \
   do {                                                                                          \
      try {                                                                                      \
         _sourceFile##ident = __FILE__;                                                          \
         _sourceLine##ident = __LINE__;                                                          \
...
#define FLOAT_MERGE_ALL(ident, merge, load)                                                      \
         FLOAT_PRINT_CURRENT_PATH(ident)                                                         \
         _isCompleteFlow##ident = NumericalDomains::FldlibBase::MergeBranches(_sourceFile##ident, _sourceLine##ident) << merge;\
      }                                                                                          \
      catch (double_fld::anticipated_termination&) {                                             \
         _isCompleteFlow##ident = false;                                                         \
         NumericalDomains::DAffine::ExecutionPath::clearSynchronizationBranches();               \
      ...
      NumericalDomains::DAffine::ExecutionPath::setFollowFlow();                                 \
      NumericalDomains::FldlibBase::splitBranchIdentifier = _currentSplitBranchIdentifier##ident;\
      _doesIterate##ident = (_mergeMemory##ident.setCurrentComplete(_isCompleteFlow##ident) << merge);\
      if (_doesIterate##ident)                                                                   \
         _doesIterate##ident = !(_saveMemory##ident.setCurrentResult(_pathExplorer##ident.isFinished(NumericalDomains::DAffine::ExecutionPath::queryMode(_oldPathExplorer##ident))) >> load);\
   } while (_doesIterate##ident);                                                                \
   NumericalDomains::DAffine::ExecutionPath::setFollowFlow(_oldDoesFollow##ident, _oldInputTraceFile##ident,\
   ...
   NumericalDomains::DAffine::ExecutionPath::setCurrentPathExplorer(_oldPathExplorer##ident);    \
   NumericalDomains::DAffine::BaseFloatAffine::splitBranches(_oldSourceInfo##ident.first, _oldSourceInfo##ident.second);\
   NumericalDomains::FldlibBase::splitBranchIdentifier = _oldSplitBranchIdentifier##ident;       \
   if (_mergeMemory##ident.isFirst())                                                            \
      NumericalDomains::DAffine::ExecutionPath::throwEmptyBranch(true); }

#define FLOAT_MERGE_ALL_WITH_COUNT(ident, merge, load, counter)                                  \
         FLOAT_PRINT_CURRENT_PATH(ident)                                                         \
         _isCompleteFlow##ident = NumericalDomains::FldlibBase::MergeBranches(_sourceFile##ident, _sourceLine##ident) << merge;\
      }                                                                                          \
      catch (double_fld::anticipated_termination&) {                                             \
      ...
      _doesIterate##ident = (_mergeMemory##ident.setCurrentComplete(_isCompleteFlow##ident) << merge);\
      if (_doesIterate##ident)                                                                   \
         _doesIterate##ident = !(_saveMemory##ident.setCurrentResult(                            \
            _pathExplorer##ident.isFinished(NumericalDomains::DAffine::ExecutionPath::queryMode(_oldPathExplorer##ident))\
               || (counter-- == 0)) >> load);                                                     \
   } while (_doesIterate##ident);                                                                \
   NumericalDomains::DAffine::ExecutionPath::setFollowFlow(_oldDoesFollow##ident, _oldInputTraceFile##ident,\
         _oldSynchronisationFile##ident, _oldSynchronisationLine##ident);                        \
...
#define FLOAT_MERGE_ALL_OUTER(ident, merge, load)                                                \
         FLOAT_PRINT_CURRENT_PATH(ident)                                                         \
         _isCompleteFlow##ident = NumericalDomains::FldlibBase::MergeBranches(_sourceFile##ident, _sourceLine##ident) << merge;\
      }                                                                                          \
      catch (double_fld::anticipated_termination&) {                                             \
...
#endif // FLOAT_AFFINE_OPTION && FLOAT_SUPPORT_INT_DOMAIN
...
#else
#include "FloatIntervalInterface.h"
...
#define long_double NumericalDomains::LongDoubleInterval
#define long_double_fld NumericalDomains::LongDoubleInterval
#endif
...
#define long_double NumericalDomains::LongDoubleExact
#define long_double_fld NumericalDomains::LongDoubleExact
#endif
